...
// import { ArrowBack, ArrowDownward, ArrowForward, ArrowUpward, Close, QuestionAnswer, Send, Email } from "@mui/icons-material";
...
//   baseURL: "https://ruziganodejs.onrender.com",
//   headers: {
...
//     try {
//       const response = await api.post("/faqs/questions", {
...
//         setStatus("success");
//         toast.success("Question sent successfully! We'll respond within 24 hours.", {
//           position: "top-right",
//           autoClose: 5000,
...
//                                       setShowModal(true);
//                                       toast.info("Question pre-filled! Add any additional details.", {
//                                         position: "top-right",
//                                         autoClose: 3000,
//                                       });
//                                     }}
...
//                 onClick={() => {
//                   setShowModal(true);
//                   toast.info("Ready to ask your question!", {
//                     position: "top-right",
//                     autoClose: 3000,
//                   });
//                 }}
...
//                       formErrors.question ? 'border-red-500 ring-2 ring-red-100' : ''
//                     }`}
//                     rows="4"
...
//                         formErrors.email ? 'border-red-500 ring-2 ring-red-100' : ''
//                       }`}
...
//                             Question sent successfully! We'll get back to you soon at {email}.
//                           </span>
//                         </div>
//                       </div>
//                     </motion.div>
...
const api = axios.create({
  ...
  withCredentials: true, // Set to false for CORS
});
...
export const FAQ = () => {
  ...
  useEffect(() => {
    const testApiConnection = async () => {
      try {
        console.log("🔍 Testing API connection to:", "https://ruziganodejs.onrender.com");
        const response = await axios.get("https://ruziganodejs.onrender.com", {
          timeout: 10000,
        });
        ...
        // Try to wake up the Render.com app
        if (error.code === 'ECONNABORTED' || error.message.includes('timeout')) {
          console.log("🔄 Render.com app might be sleeping. Trying to wake it up...");
  ...
  const handleSubmit = async () => {
    ...
    try {
      // First, let's try to wake up the Render.com app if it's sleeping
      if (apiStatus === "offline") {
        console.log("🔄 Attempting to wake up Render.com app...");
        try {
          await axios.get("https://ruziganodejs.onrender.com", { timeout: 5000 });
          console.log("✅ App seems to be awake now");
        } catch (wakeError) {
          console.log("⚠️ Could not wake app, continuing anyway...");
        }
      }

      // Try the POST request with retry logic
      let retryCount = 0;
      const maxRetries = 2;
      let lastError = null;
      ...
      while (retryCount <= maxRetries) {
        try {
          ...
          const response = await api.post("/frequent/question", questionData);
          ...
          setApiStatus("online"); // Update API status
          ...
          if (retryCount <= maxRetries) {
            console.log(`🔄 Retry ${retryCount}/${maxRetries} after error:`, err.message);
      ...
      if (err.code === 'ECONNABORTED' || err.message.includes('timeout')) {
        errorMessage = "The server is taking too long to respond. This might be because the Render.com app is waking up from sleep mode. Please try again in 30 seconds.";
        ...
        if (err.response.status === 400) {
          ...
          errorMessage = "The FAQ submission endpoint was not found. Please contact support.";
        } else if (err.response.status === 500) {
          errorMessage = "Server error. Please try again later.";
        } else if (err.response.status === 503) {
          errorMessage = "Service temporarily unavailable. The server might be restarting. Please try again in a moment.";
        }
      } else if (err.request) {
        // The request was made but no response was received
        errorMessage = "No response received from server. The server might be down or your internet connection is unstable.";
      }
            ...
            >
              <div className="p-4 sm:p-6 md:p-8">
                ...
                <div className="mb-4 sm:mb-6">
                  ...
                  <p className="text-xs sm:text-sm text-gray-500 mt-2">
                    Please provide as much detail as possible for a
                ...
                {/* Render.com status warning */}
                {apiStatus === "offline" && (
                  <div className="mb-4 p-3 bg-yellow-50 border border-yellow-200 rounded-lg">
                    <p className="text-yellow-700 text-sm">
                      ⚠️ <strong>Note:</strong> The server is currently starting up. 
                      First submission may take up to 30 seconds due to Render.com's cold start.
                    </p>
                  </div>
                )}
