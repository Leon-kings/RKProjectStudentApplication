...
const api = axios.create({
  baseURL: "https://ruziganodejs.onrender.com", // Replace with your actual API base URL
  headers: {
...
export const FAQ = () => {
  ...
  // Function to submit question using Axios
  const handleSubmit = async () => {
    if (!question.trim()) {
      setStatus("fail");
      return;
    ...
    try {
      ...
      const questionData = {
        ...
        category: "user-submitted", // You can modify this based on your needs
        status: "pending",
        // You can add more fields as needed
      };

      // Send POST request using Axios
      const response = await api.post("/submit-question", questionData, {
        // You can add additional headers if needed
        headers: {
          "Authorization": `Bearer ${localStorage.getItem("token")}`, // Example if you have authentication
        },
      });
      ...
      if (response.status === 200 || response.status === 201) {
        ...
        // Optional: You might want to add the new question to the FAQ list
        // const newFaq = {
        //   id: faqs.length + 1,
        //   question: questionData.question,
        //   answer: "Our team will respond to this question soon.",
        //   category: "pending",
        //   expanded: false
        // };
        // faqs.push(newFaq); // Or use state if you want to update the UI
  ...
  // Optional: Function to fetch FAQs from API (if you want to load them dynamically)
  const fetchFAQs = async () => {
    try {
      const response = await api.get("/faqs");
      // You would then set the FAQs in state
