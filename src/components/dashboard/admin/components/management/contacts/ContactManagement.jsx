...
const getCategoryBadgeColor = (category) => {
  switch (category) {
    ...
    case 'partnership': return 'bg-cyan-100 text-cyan-800';
    case 'career': return 'bg-amber-100 text-amber-800';
    default: return 'bg-gray-100 text-gray-800';
  }
};
...
const ContactCard = React.memo(({ contact, onView, onReply, onEdit, onDelete }) => {
  ...
  return (
    ...
    >
      <div className="p-6">
        ...
        <h4 className="text-md font-semibold text-gray-800 mb-2 truncate">{contact.subject}</h4>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          <span className={`px-2 py-1 rounded-full text-xs font-bold ${getCategoryBadgeColor(contact.category)}`}>
            {contact.category.toUpperCase()}
          </span>
          <span className={`px-2 py-1 rounded-full text-xs font-bold ${getPriorityBadgeColor(contact.priority)}`}>
            {contact.priority.toUpperCase()}
          </span>
          <span className={`px-2 py-1 rounded-full text-xs font-bold ${getStatusBadgeColor(contact.status)}`}>
...
const Pagination = React.memo(({ 
  ...
  }, [indexOfLastContact, totalContacts, currentPage, totalPages, onPageChange]);
    ...
    <div className="flex items-center justify-between mt-8">
      <div className="text-sm text-gray-600">
        ...
        {indexOfLastContact >= totalContacts && currentPage < totalPages && (
          <span className="ml-2 text-blue-500 font-medium">(Auto-advancing to next page)</span>
        )}
      </div>
...
export const ContactManagement = () => {
  ...
  const generateSampleContacts = useCallback(() => {
    const sampleContacts = [
      {
        ...
        message: 'I really like your new website design. The user interface is very intuitive and easy to navigate. However, I noticed that the contact form on the pricing page is not working properly on mobile devices.',
        category: 'feedback',
        status: 'read',
        priority: 'normal',
      ...
      },
      {
        ...
        message: 'I am having issues with the API integration. Getting 500 error when trying to POST data to the /users endpoint. Can you please help me resolve this issue as soon as possible?',
        category: 'support',
        status: 'unread',
        priority: 'high',
      ...
      },
      {
        ...
        message: 'We are interested in exploring partnership opportunities with your company. Our company specializes in enterprise software solutions and we believe there is great synergy between our offerings.',
      ...
      },
      {
        ...
        message: 'I am interested in your enterprise plan but have some questions about the pricing structure. Could you please provide more details about the annual billing discount and what features are included?',
        category: 'sales',
        status: 'read',
        priority: 'normal',
      ...
      },
      {
        ...
        message: 'Our entire system has been down for 3 hours now and this is affecting our daily operations. This is an emergency situation that needs immediate attention. Please contact me as soon as possible.',
        category: 'complaint',
        status: 'unread',
        priority: 'urgent',
      ...
      },
      {
        ...
        message: 'I saw your job posting for a Senior React Developer and would like to apply. I have 5+ years of experience with React, Node.js, and MongoDB. When can I schedule an interview?',
        category: 'career',
        status: 'read',
        priority: 'normal',
      ...
      },
      {
        ...
        message: 'I would like to know more about your services and request a demo of your platform. Can you please send me more information and available time slots for next week?',
        category: 'general',
        status: 'read',
        priority: 'low',
      ...
      },
      {
        ...
        message: 'I found a bug in the mobile app version 2.1.0. When trying to upload multiple images, the app crashes on iOS devices. This happens consistently when selecting more than 5 photos.',
        category: 'support',
        status: 'read',
        priority: 'high',
      ...
      },
      {
        ...
        message: 'I would like to request a dark mode feature for your dashboard. Many users work late hours and a dark theme would be much easier on the eyes. Consider this for your next update.',
        category: 'feedback',
        status: 'unread',
        priority: 'normal',
      ...
      },
      {
        ...
        message: 'When exporting data to CSV format, some special characters are not being encoded properly. This causes issues when importing the data into other systems. Please fix this issue.',
        category: 'support',
        status: 'read',
        priority: 'normal',
    ...
    const moreContacts = [];
    const names = ['Alex Turner', 'Sophia Williams', 'Daniel Lee', 'Olivia Martinez', 'William Clark'];
    const companies = ['Innovation Labs', 'Cloud Systems', 'Data Analytics Co', 'Mobile Solutions', 'Web Development Inc'];
    ...
    for (let i = 11; i <= 25; i++) {
      ...
      moreContacts.push({
        ...
        replyDate: randomReplied ? new Date(Date.now() - Math.random() * 3 * 24 * 60 * 60 * 1000).toISOString() : null
      });
    }
  ...
  // Add new contact
  const handleAddContact = useCallback(async (e) => {
    e.preventDefault();
  ...
  // Update contact
  const handleUpdateContact = useCallback(async (e) => {
    e.preventDefault();
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.put(`${API_BASE_URL}/contacts/${selectedContact._id}`, data, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleDeleteContact = useCallback((contactId) => {
    showConfirm('Are you sure you want to delete this contact message? This action cannot be undone.', async () => {
      try {
        const token = localStorage.getItem('token');
  ...
  const handleSendReply = useCallback(async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(`${API_BASE_URL}/contacts/${selectedContact._id}/reply`, data, {
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
        }
  ...
  const handleExportContacts = useCallback(() => {
    const headers = ['ID', 'Name', 'Email', 'Phone', 'Company', 'Subject', 'Category', 'Status', 'Priority', 'Created At', 'Replied'];
        ...
        >
          <div className="p-6">
            <div className="flex justify-between items-start mb-6">
              <div>
                <h2 className="text-2xl font-bold text-gray-900 mb-1">{selectedContact.subject}</h2>
                <div className="flex items-center space-x-2">
                  <span className={`px-3 py-1 rounded-full text-sm font-bold ${getPriorityBadgeColor(selectedContact.priority)}`}>
                    {selectedContact.priority.toUpperCase()}
                  </span>
                  <span className={`px-3 py-1 rounded-full text-sm font-bold ${getStatusBadgeColor(selectedContact.status)}`}>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl">
                    <div className="flex items-center">
                      ...
                      <span className="text-gray-700">{selectedContact.company || 'Not specified'}</span>
                    </div>
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl">
                    <div className="flex items-center mb-2">
                      ...
                      <span className="text-gray-700">Category: <span className="font-semibold">{selectedContact.category}</span></span>
                    </div>
                    <div className="flex items-center mb-2">
                      <CalendarTodayIcon className="h-5 w-5 text-gray-500 mr-2" />
                      <span className="text-gray-700">Received: {new Date(selectedContact.createdAt).toLocaleString()}</span>
                    </div>
                    {selectedContact.replyDate && (
                      <div className="flex items-center">
                        <ReplyIcon className="h-5 w-5 text-gray-500 mr-2" />
                        <span className="text-gray-700">Replied: {new Date(selectedContact.replyDate).toLocaleString()}</span>
                      </div>
                    )}
                  </div>
                </div>
        ...
        >
          <div className="p-6">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">Reply to {selectedContact.name}</h2>
                <p className="text-gray-600 mt-1">Send a response to this contact message</p>
              </div>
              <motion.button
                onClick={() => {
                  setShowReplyModal(false);
      ...
      >
        ...
        >
          <div className="p-6">
            <div className="flex justify-between items-center mb-6">
              <div>
                ...
                <p className="text-gray-600 mt-1">
                  {isEditMode ? 'Update contact information' : 'Add a new contact message'}
                </p>
              </div>
              <motion.button
                onClick={() => {
            ...
            <form onSubmit={isEditMode ? handleUpdateContact : handleAddContact} className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <label className="block text-sm font-semibold text-gray-700 mb-2">
                    Name *
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          <div className="flex flex-col md:flex-row md:items-center md:justify-between">
            <div>
              <h1 className="text-4xl sm:text-5xl font-bold text-gray-900 mb-2">Contact Management</h1>
              <p className="text-lg text-gray-600">Manage contact messages, inquiries, and support requests</p>
            </div>
