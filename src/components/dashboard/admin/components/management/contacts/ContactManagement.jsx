    ...
    return (
      ...
      >
        <div className="p-6">
          ...
          <p className="text-gray-600 text-sm mb-4 line-clamp-2">
            {contact.message && contact.message.length > 150
              ? `${contact.message.substring(0, 150)}...`
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            >
              {contact.priority?.toUpperCase() || "NORMAL"}
            </span>
            <span
              className={`px-2 py-1 rounded-full text-xs font-bold ${getStatusBadgeColor(
                contact.status
            ...
            >
              {contact.status?.toUpperCase() || "UNREAD"}
            </span>
            {contact.replied && (
              <span className="px-2 py-1 rounded-full text-xs font-bold bg-green-100 text-green-800">
                REPLIED
...
export const ContactManagement = () => {
    ...
    try {
      ...
      if (error.response) {
        ...
        if (error.response.status === 401) {
          ...
          showError("Contacts endpoint not found. Please check API configuration.");
        } else {
        ...
        // Other errors
        showError(error.message || "Failed to fetch contacts");
      }
      setContacts([]);
      setFilteredContacts([]);
    } finally {
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        (contact) =>
          ...
          (contact.subject?.toLowerCase().includes(query) || false) ||
          (contact.message?.toLowerCase().includes(query) || false) ||
  ...
  const handleUpdateContact = useCallback(
    async (e) => {
      ...
      if (!selectedContact || !selectedContact._id) {
        showError("No contact selected for update");
        return;
      }
  ...
  const handleReplyContact = useCallback((contact) => {
    if (!contact || !contact._id) {
      showError("Invalid contact for reply");
      return;
    }
    ...
    const updatedReplyData = {
      ...
      message: `\n\n--- Original Message ---\nFrom: ${contact.name || "Unknown"} <${
        contact.email || "No email"
      }>\nDate: ${contact.createdAt ? new Date(contact.createdAt).toLocaleString() : "Unknown date"}\nSubject: ${
        contact.subject || "No subject"
      }\n\n${contact.message || "No message"}`,
      cc: "",
      bcc: "",
  ...
  const handleSendReply = useCallback(
    async (e) => {
      ...
      if (!selectedContact || !selectedContact._id) {
        showError("No contact selected for reply");
        return;
      }
  ...
  const handleExportContacts = useCallback(() => {
    ...
    showSuccess("Contacts exported successfully!");
  }, [filteredContacts, showError, showSuccess]);
            ...
            <div className="flex justify-between items-start mb-6">
              <div>
                ...
                <div className="flex items-center space-x-2">
                  ...
                  >
                    {selectedContact.priority?.toUpperCase() || "NORMAL"}
                  </span>
                  <span
                    className={`px-3 py-1 rounded-full text-sm font-bold ${getStatusBadgeColor(
                      selectedContact.status
                  ...
                  >
                    {selectedContact.status?.toUpperCase() || "UNREAD"}
                  </span>
                  {selectedContact.replied && (
                    <span className="px-3 py-1 rounded-full text-sm font-bold bg-green-100 text-green-800">
                      REPLIED
                  ...
                  <div className="bg-gray-50 p-4 rounded-xl">
                    <div className="flex items-center mb-2">
                      ...
                      <span className="text-gray-700">
                        Category:{" "}
                        <span className="font-semibold">
                          {selectedContact.category || "general"}
                        </span>
                      </span>
                    </div>
                    <div className="flex items-center mb-2">
        ...
        >
          <div className="p-6">
            <div className="flex justify-between items-center mb-6">
              <div>
                <h2 className="text-2xl font-bold text-gray-900">
                  Reply to {selectedContact.name || "Contact"}
                </h2>
                <p className="text-gray-600 mt-1">
                  Send a response to this contact message
                </p>
