...
// Use relative path for API to avoid CORS issues
const API_BASE_URL = "https://ruziganodejs.onrender.com"; // This will proxy through Vite
...
const MOCK_AIRPORT_BOOKINGS = [
  {
    ...
    flightNumber: "WB 345",
    airline: "RwandAir",
    arrivalDate: "2024-03-15T00:00:00.000Z",
    arrivalTime: "14:30",
    departureDate: "2024-03-20T00:00:00.000Z",
    departureTime: "10:15",
    airport: "Kigali International Airport",
    terminal: "Terminal 1",
    serviceType: "standard",
    ...
    statistics: {
      emailsSent: 2,
      statusChanges: 1,
      lastEmailSent: "2024-01-16T00:00:00.000Z",
      totalRevenue: 75
    }
  },
];

// Update SERVICE_TYPES to match Mongoose schema
const SERVICE_TYPES = [
...
// Update STATUS_OPTIONS to match Mongoose schema
const STATUS_OPTIONS = [
  { value: "pending", label: "Pending", color: "yellow" },
  { value: "confirmed", label: "Confirmed", color: "green" },
  { value: "completed", label: "Completed", color: "blue" },
...
// Update PAYMENT_METHODS to match Mongoose schema
const PAYMENT_METHODS = [
  { value: "credit_card", label: "Credit Card" },
  { value: "bank_transfer", label: "Bank Transfer" },
  { value: "paypal", label: "PayPal" },
...
export const AirportBookingManagement = () => {
  ...
  // Filter and sort bookings - ADDED SAFETY CHECK
  useEffect(() => {
    ...
    if (dateFilter) {
      filtered = filtered.filter(
        (booking) => {
          ...
          const bookingDate = new Date(booking.arrivalDate).toISOString().split('T')[0];
  ...
  // Fetch bookings using Axios with fallback
  const fetchBookings = async () => {
    setLoading(true);
    setInitialLoading(true);
    try {
      const response = await api.get("/airport/booking");
      ...
      // Try to fetch statistics if API exists
      try {
        await fetchStatistics();
      } catch (statsError) {
        console.log("Statistics endpoint not available, using local calculation");
      }
      
    } catch (error) {
      console.error("Error fetching bookings:", error);
      showNotification("Failed to load bookings from server. Using demo data.", "warning");
      
      // Fallback to mock data
      setBookings(MOCK_AIRPORT_BOOKINGS);
      setFilteredBookings(MOCK_AIRPORT_BOOKINGS);
  ...
  const fetchStatistics = async () => {
    try {
      const response = await api.get("/airport/booking/statistics");
      setStatistics(response.data);
    } catch (error) {
      console.log("Statistics endpoint not available, calculating locally");
      calculateLocalStatistics();
    }
  };
  ...
  const calculateLocalStatistics = () => {
    ...
    const overview = {
      ...
      avgPassengers: bookings.reduce((sum, booking) => sum + (parseInt(booking.numberOfPassengers) || 0), 0) / bookings.length,
      avgBags: bookings.reduce((sum, booking) => sum + (parseInt(booking.numberOfBags) || 0), 0) / bookings.length,
      totalEmailsSent: bookings.reduce((sum, booking) => sum + ((booking.statistics?.emailsSent) || 0), 0)
    };
    ...
    bookings.forEach(booking => {
      ...
      if (!byStatus[booking.status]) {
        byStatus[booking.status] = { count: 0, totalAmount: 0 };
      }
      byStatus[booking.status].count++;
      byStatus[booking.status].totalAmount += parseFloat(booking.totalAmount) || 0;

      // Group by service type
      if (!byServiceType[booking.serviceType]) {
        byServiceType[booking.serviceType] = { count: 0, totalAmount: 0 };
      }
      byServiceType[booking.serviceType].count++;
      byServiceType[booking.serviceType].totalAmount += parseFloat(booking.totalAmount) || 0;
    });
    ...
    setStatistics({
      ...
      byServiceType: Object.entries(byServiceType).map(([key, value]) => ({
        _id: key,
        count: value.count,
  ...
  // Update calculateLocalStatistics when bookings change
  useEffect(() => {
  ...
  const handleInputChange = (e) => {
    ...
    setBookingForm((prev) => ({
      ...prev,
      [name]: name === 'numberOfPassengers' || name === 'numberOfBags' 
        ? parseInt(value) || 0 
        : value,
    }));
  ...
  const validateForm = () => {
    ...
    if (!bookingForm.phone.trim()) newErrors.phone = "Phone number is required";
    if (!bookingForm.nationality.trim()) newErrors.nationality = "Nationality is required";
    if (!bookingForm.flightNumber.trim())
      newErrors.flightNumber = "Flight number is required";
    if (!bookingForm.airline.trim()) newErrors.airline = "Airline is required";
    if (!bookingForm.arrivalDate)
    ...
    if (!bookingForm.departureTime)
      newErrors.departureTime = "Departure time is required";
    if (!bookingForm.airport.trim()) newErrors.airport = "Airport is required";
    if (!bookingForm.terminal.trim()) newErrors.terminal = "Terminal is required";
    if (!bookingForm.serviceType)
      newErrors.serviceType = "Service type is required";
    if (!bookingForm.numberOfPassengers || bookingForm.numberOfPassengers < 1)
      newErrors.numberOfPassengers = "Number of passengers is required (min: 1)";
    ...
    // Number validation
    if (bookingForm.numberOfPassengers > 50) {
      newErrors.numberOfPassengers = "Maximum 50 passengers allowed";
    }
  ...
  // Handle form submission (Create/Update) with offline support
  const handleSubmit = async (e) => {
    if (e) e.preventDefault();
    ...
    try {
      ...
      const bookingData = {
        ...bookingForm,
        arrivalDate: new Date(bookingForm.arrivalDate).toISOString(),
        departureDate: new Date(bookingForm.departureDate).toISOString(),
        numberOfPassengers: parseInt(bookingForm.numberOfPassengers),
        numberOfBags: parseInt(bookingForm.numberOfBags) || 0,
        totalAmount: parseFloat(calculateTotalAmount(bookingForm)),
        currency: "USD",
      };
      ...
      if (isEditMode && selectedBooking) {
        ...
        try {
          const response = await api.put(
            `/airport/booking/${selectedBooking._id}`,
            bookingData
          );
          ...
          newBooking = {
            ...
            bookingReference: selectedBooking.bookingReference,
            status: selectedBooking.status,
            statistics: selectedBooking.statistics,
            createdAt: selectedBooking.createdAt,
        ...
        const updatedBookings = bookings.map((booking) =>
          booking._id === selectedBooking._id ? newBooking : booking
        );
        setBookings(updatedBookings);
        showNotification("Airport booking updated successfully!", "success");
      } else {
        // Create new booking
        try {
          const response = await api.post("/airport/booking", bookingData);
          newBooking = response.data;
          ...
          const newId = bookings.length > 0 
            ? Math.max(...bookings.map(b => parseInt(b._id) || 0)) + 1 
            : 1;
          
          newBooking = {
            ...bookingData,
            _id: newId.toString(),
            bookingReference: `AIR-${String(bookings.length + 1).padStart(3, '0')}`,
            status: "pending",
            statistics: {
              emailsSent: 0,
              statusChanges: 0,
              totalRevenue: bookingData.totalAmount
            },
  ...
  const calculateTotalAmount = (formData) => {
    ...
    const baseRate = baseRates[formData.serviceType] || 50;
    const passengerMultiplier = parseInt(formData.numberOfPassengers) || 1;
    const bagMultiplier = parseInt(formData.numberOfBags) || 0;
    
    // Additional charges for bags
    const bagCharge = bagMultiplier > 2 ? (bagMultiplier - 2) * 10 : 0;
    
    return (baseRate * passengerMultiplier + bagCharge).toFixed(2);
  };

  // Handle delete booking with fallback
  const handleDelete = async () => {
    setLoading(true);
    ...
    try {
      if (selectedBooking?._id) {
        try {
          await api.delete(`/airport/booking/${selectedBooking._id}`);
        } catch (error) {
  ...
  // Handle status change with fallback
  const handleStatusChange = async (newStatus) => {
    setLoading(true);
    ...
    try {
      ...
      if (selectedBooking?._id) {
        try {
          const response = await api.patch(`/airport/booking/${selectedBooking._id}/status`, {
            status: newStatus,
          });
          ...
          updatedBooking = {
            ...
            status: newStatus,
            statistics: {
              ...selectedBooking.statistics,
              statusChanges: (selectedBooking.statistics?.statusChanges || 0) + 1
            },
            updatedAt: new Date().toISOString(),
          };
        }
  ...
  const handleSendEmail = async () => {
    ...
    try {
      ...
      // Update booking with email count
      const updatedBooking = {
        ...selectedBooking,
        statistics: {
          ...selectedBooking.statistics,
          emailsSent: (selectedBooking.statistics?.emailsSent || 0) + 1,
      ...
      showNotification("Email sent (simulated)", "success");
      setOpenEmailDialog(false);
    } finally {
      setLoading(false);
    }
  ...
  const handleEdit = (booking) => {
    ...
    const formattedBooking = {
      ...booking,
      arrivalDate: booking.arrivalDate ? new Date(booking.arrivalDate).toISOString().split('T')[0] : "",
      ...
      numberOfBags: booking.numberOfBags?.toString() || "0",
    };
    setBookingForm(formattedBooking);
    setIsEditMode(true);
    setOpenDialog(true);
  };
  ...
  const bookingStatistics = {
    ...
    pending: Array.isArray(bookings) ? bookings.filter((booking) => booking.status === "pending").length : 0,
    confirmed: Array.isArray(bookings) ? bookings.filter((booking) => booking.status === "confirmed").length : 0,
    revenue: Array.isArray(bookings) ? bookings
      .filter((booking) => booking.status !== "cancelled")
      .reduce((sum, booking) => sum + parseFloat(booking.totalAmount || 0), 0) : 0,
    totalRevenue: statistics.overview?.totalRevenue || 0,
    avgPassengers: statistics.overview?.avgPassengers?.toFixed(1) || 0,
  };
  ...
  const paginatedBookings = Array.isArray(filteredBookings) 
    ...
    : [];
  const totalPages = Math.ceil((Array.isArray(filteredBookings) ? filteredBookings.length : 0) / rowsPerPage);
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
      {/* <Sidebar onToggleNotifications={() => setShowNotificationsModal(!showNotificationsModal)} /> */}
      ...
      <div className="flex-1 p-2 sm:p-3 md:p-4 lg:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div>
              ...
              <p className="text-xs sm:text-sm md:text-base text-gray-600">
                Manage airport transfers, meet & greet services, and VIP arrangements
              </p>
            </div>
            <button
              onClick={handleCreate}
              className="flex items-center justify-center px-3 py-2 sm:px-4 sm:py-2.5 md:px-5 md:py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:from-blue-700 hover:to-blue-800 shadow hover:shadow-lg transition-all w-full sm:w-auto text-sm sm:text-base font-medium"
            >
              <AddIcon className="w-4 h-4 sm:w-5 sm:h-5 md:w-6 md:h-6 mr-1 sm:mr-2" />
        ...
        {/* Statistics Cards - Responsive Grid */}
        <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 md:gap-6 mb-4 sm:mb-6 md:mb-8">
          <div className="bg-gradient-to-r from-blue-500 to-blue-600 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 text-white shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm font-medium opacity-90">Total Bookings</p>
                <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold mt-1 sm:mt-2">
                  {bookingStatistics.total}
                </p>
              </div>
              <FlightTakeoffIcon className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 opacity-80" />
            </div>
          </div>

          <div className="bg-gradient-to-r from-yellow-500 to-yellow-600 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 text-white shadow">
            <div className="flex items-center justify-between">
              <div>
              ...
              <ScheduleIcon className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 opacity-80" />
            </div>
          </div>

          <div className="bg-gradient-to-r from-green-500 to-green-600 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 text-white shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm font-medium opacity-90">Confirmed</p>
                <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold mt-1 sm:mt-2">
                  {bookingStatistics.confirmed}
                </p>
              </div>
              <CheckIcon className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 opacity-80" />
            </div>
          </div>

          <div className="bg-gradient-to-r from-purple-500 to-purple-600 rounded-lg sm:rounded-xl p-3 sm:p-4 md:p-6 text-white shadow">
            <div className="flex items-center justify-between">
              <div>
                <p className="text-xs sm:text-sm font-medium opacity-90">Total Revenue</p>
                <p className="text-lg sm:text-xl md:text-2xl lg:text-3xl font-bold mt-1 sm:mt-2">
                  ${(bookingStatistics.totalRevenue || bookingStatistics.revenue).toLocaleString()}
                </p>
              </div>
              <PaymentIcon className="w-6 h-6 sm:w-8 sm:h-8 md:w-10 md:h-10 lg:w-12 lg:h-12 opacity-80" />
            </div>
          </div>
        </div>

        {/* Additional Statistics - Only show if we have data */}
        {(statistics.byStatus?.length > 0 || statistics.byServiceType?.length > 0) && (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
                  ...
                  {statistics.byStatus?.map((item) => (
                    <div key={item._id} className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <span className="text-xs text-gray-500">${item.totalAmount?.toLocaleString()}</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            {statistics.byServiceType?.length > 0 && (
              <div className="bg-white rounded-lg shadow p-4">
                <h3 className="text-lg font-semibold text-gray-900 mb-3">Service Type Distribution</h3>
                <div className="space-y-2">
                  {statistics.byServiceType?.map((item) => (
                    <div key={item._id} className="flex items-center justify-between">
                      <span className="text-sm text-gray-700">
                        {SERVICE_TYPES.find(s => s.value === item._id)?.label || item._id}
                      </span>
                      <div className="flex items-center gap-3">
                        <span className="font-semibold">{item.count}</span>
                        <span className="text-xs text-gray-500">${item.avgAmount?.toFixed(2)} avg</span>
                      </div>
                    </div>
                  ))}
                </div>
            ...
            <div className="w-full">
              <div className="relative">
                ...
                <input
                  ...
                  value={searchTerm}
                  onChange={(e) => setSearchTerm(e.target.value)}
                  className="w-full pl-9 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition-all"
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
              <div>
                <div className="relative">
                  ...
                  <select
                    value={statusFilter}
                    onChange={(e) => setStatusFilter(e.target.value)}
                    className="w-full pl-9 sm:pl-10 pr-3 sm:pr-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none appearance-none bg-white"
                  >
                    <option value="all">All Status</option>
                    {STATUS_OPTIONS.map((status) => (
                      <option key={status.value} value={status.value}>
              ...
              <div>
                <select
                  value={serviceFilter}
                  onChange={(e) => setServiceFilter(e.target.value)}
                  className="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none appearance-none bg-white"
                >
                  <option value="all">All Services</option>
                  {SERVICE_TYPES.map((service) => (
                    <option key={service.value} value={service.value}>
              ...
              <div>
                <input
                  ...
                  value={dateFilter}
                  onChange={(e) => setDateFilter(e.target.value)}
                  className="w-full px-3 sm:px-4 py-2 sm:py-3 text-sm sm:text-base border border-gray-300 rounded-lg focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none"
                />
              </div>
              ...
              <div className="flex gap-2 sm:gap-3">
                <button
                  ...
                  className="flex-1 flex items-center justify-center px-3 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors text-sm sm:text-base"
                >
                ...
                <button
                  ...
                  disabled={loading}
                  className="flex-1 flex items-center justify-center px-3 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 transition-colors text-sm sm:text-base disabled:opacity-50"
                >
                  <RefreshIcon className="w-4 h-4 mr-1 sm:mr-2" />
                    ...
                    {!Array.isArray(paginatedBookings) || paginatedBookings.length === 0 ? (
                      <tr>
                        ...
                        >
                          <div className="flex flex-col items-center">
                            ...
                            <button
                              onClick={handleCreate}
                              className="mt-4 px-4 py-2 sm:px-5 sm:py-3 bg-gradient-to-r from-blue-600 to-blue-700 text-white rounded-lg hover:bg-blue-700 shadow text-sm sm:text-base"
                            >
                              Create New Booking
                            </button>
                          </div>
                      ...
                      paginatedBookings.map((booking) => (
                        ...
                        >
                          <td className="px-3 sm:px-4 md:px-6 py-4">
                            <div className="flex items-center">
                              <div className="flex-shrink-0 h-8 w-8 sm:h-10 sm:w-10 md:h-12 md:w-12 bg-gradient-to-r from-blue-400 to-purple-400 rounded-lg flex items-center justify-center text-white font-bold text-sm sm:text-base">
              ...
              <div className="p-3 sm:p-4 md:p-6 lg:p-8 overflow-y-auto max-h-[calc(90vh-4rem)]">
                ...
                >
                  ...
                  <div>
                    ...
                    {/* Estimated Total */}
                    <div className="mt-4 sm:mt-6 p-3 sm:p-4 bg-gradient-to-r from-blue-50 to-blue-100 rounded-lg sm:rounded-xl">
                      <div className="flex items-center justify-between">
                        <div>
                          ...
                          <p className="text-xs text-gray-500">
                            Based on service type and passengers
                          </p>
                        </div>
                        <div className="text-lg sm:text-xl font-bold text-blue-700">
                          ${calculateTotalAmount(bookingForm)}
                        </div>
                      </div>
                    </div>
                  </div>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-50 p-2 sm:p-3 md:p-4">
            <div className="bg-white rounded-lg sm:rounded-xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-4">
                    <div className="bg-blue-50 rounded-lg p-2 sm:p-3 text-center">
                      ...
                      <p className="text-lg sm:text-xl font-bold text-blue-700">
                        {selectedBooking.statistics?.emailsSent || 0}
                      </p>
                    </div>
                    <div className="bg-green-50 rounded-lg p-2 sm:p-3 text-center">
                      <p className="text-xs text-gray-600">Status Changes</p>
                      <p className="text-lg sm:text-xl font-bold text-green-700">
                        {selectedBooking.statistics?.statusChanges || 0}
                      </p>
                    </div>
              ...
              <div className="px-4 sm:px-6 md:px-8 py-3 sm:py-4 border-t border-gray-200 flex flex-col sm:flex-row justify-between gap-3">
                <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
                  <button
                    onClick={() => handleEmailClick(selectedBooking)}
                    className="px-4 py-2 sm:px-5 sm:py-3 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-lg sm:rounded-xl hover:from-indigo-700 hover:to-indigo-800 shadow-md flex items-center justify-center transition-all text-sm sm:text-base"
                  >
      ...
      {/* Email Send Modal - Responsive */}
      {openEmailDialog && selectedBooking && (
        <>
          <ModalBackdrop onClose={handleCloseEmailDialog} />
          <div className="fixed inset-0 flex items-center justify-center z-50 p-2 sm:p-3 md:p-4">
            <div className="bg-white rounded-lg sm:rounded-xl shadow-2xl w-full max-w-sm sm:max-w-md">
              <div className="p-4 sm:p-6 md:p-8">
                <div className="flex items-center justify-center w-12 h-12 sm:w-16 sm:h-16 bg-indigo-100 rounded-full mx-auto mb-4 sm:mb-6">
                  <MailIcon className="w-6 h-6 sm:w-8 sm:h-8 text-indigo-600" />
                </div>
                <h3 className="text-lg sm:text-xl font-bold text-gray-900 text-center mb-3 sm:mb-4">
                ...
                <div className="bg-blue-50 border border-blue-200 rounded-lg sm:rounded-xl p-3 sm:p-4 mb-4 sm:mb-6">
                  <div className="flex">
                    <InfoIcon className="w-4 h-4 sm:w-5 sm:h-5 text-blue-500 flex-shrink-0 mt-0.5" />
                    <div className="ml-3">
                ...
                <div className="space-y-2 mb-4 sm:mb-6">
                  ...
                  {selectedBooking.statistics?.lastEmailSent && (
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Last email:</span>
                      <span className="font-semibold">
                        {formatDate(selectedBooking.statistics.lastEmailSent)}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="flex flex-col sm:flex-row justify-center space-y-3 sm:space-y-0 sm:space-x-4">
                  ...
                  <button
                    ...
                    disabled={loading}
                    className="px-4 py-2 sm:px-5 sm:py-3 bg-gradient-to-r from-indigo-600 to-indigo-700 text-white rounded-lg sm:rounded-xl hover:from-indigo-700 hover:to-indigo-800 shadow-md disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center transition-all text-sm sm:text-base"
                  >
                    {loading ? (
                      <>
                        <div className="animate-spin rounded-full h-4 w-4 sm:h-5 sm:w-5 border-b-2 border-white mr-2"></div>
