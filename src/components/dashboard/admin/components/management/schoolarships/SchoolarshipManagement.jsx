...
const SCHOLARSHIP_TYPES = [
  'Full Scholarship', 'Partial Scholarship', 'Merit-based Scholarship',
  'Need-based Scholarship', 'Sports Scholarship', 'Research Scholarship',
  'Merit-based', 'Need-based', 'Athletic', 'Research', 'Government', 'University', 'External'
];
...
export const ScholarshipManagement = () => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        const transformedData = result.data.map(app => ({
          ...
          scholarshipInterest: app.scholarshipInterest || app.scholarshipType || '',
      ...
      showNotification('Error fetching applications. Please check your API connection.', 'error');
    } finally {
      setApiLoading(false);
    }
  };
  ...
  const statistics = {
    total: applications.length,
    pending: applications.filter(app => app.status === 'pending' || app.status === 'Submitted' || app.status === 'Draft').length,
              ...
              {paginatedApplications.length === 0 ? (
                ...
                paginatedApplications.map((app) => (
                  <tr key={app.id} className="hover:bg-gray-50">
                    ...
                    <td className="px-6 py-4">
                      <div className="text-sm text-gray-900">{app.targetProgram || 'Not specified'}</div>
                      <div className="text-xs text-gray-500 mt-1">GPA: {app.gpa || 'N/A'}</div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div>
                        {getStatusBadge(app.status)}
            ...
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl max-h-[90vh] overflow-hidden">
              ...
              <div className="p-6 overflow-y-auto max-h-[calc(90vh-8rem)]">
                <div className="space-y-6">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="flex-shrink-0 h-16 w-16 bg-gradient-to-r from-blue-500 to-purple-500 rounded-full flex items-center justify-center text-white text-2xl font-bold">
                      {selectedApplication.firstName?.charAt(0) || 'A'}{selectedApplication.lastName?.charAt(0) || 'P'}
                    </div>
                    <div>
                      <h3 className="text-2xl font-bold text-gray-900">
                        {selectedApplication.firstName} {selectedApplication.lastName}
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm text-gray-500">Current Education</p>
                        <p className="font-medium">{selectedApplication.currentEducation || 'Not specified'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">GPA</p>
                      ...
                      <div>
                        <p className="text-sm text-gray-500">Target University</p>
                        <p className="font-medium">{selectedApplication.targetUniversity || 'Not specified'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Target Country</p>
                        <p className="font-medium">{selectedApplication.targetCountry || 'Not specified'}</p>
                      </div>
                      <div className="col-span-2">
                        <p className="text-sm text-gray-500">Target Program</p>
                        <p className="font-medium">{selectedApplication.targetProgram || 'Not specified'}</p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-lg p-4">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div>
                        <p className="text-sm text-gray-500">Scholarship Interest</p>
                        <p className="font-medium">{selectedApplication.scholarshipInterest || 'Not specified'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Intake Year</p>
                        <p className="font-medium">{selectedApplication.intakeYear || 'Not specified'}</p>
                      </div>
                      <div>
                        <p className="text-sm text-gray-500">Documents Status</p>
                        <p className={`font-medium ${
                  ...
                  <div className="bg-blue-50 rounded-lg p-4">
                    <h4 className="text-lg font-medium text-gray-900 mb-3">Update Status</h4>
                    <div className="flex space-x-2">
                      ...
                      {selectedApplication.status !== 'pending' && selectedApplication.status !== 'Submitted' && (
                        <button
                          onClick={() => {
